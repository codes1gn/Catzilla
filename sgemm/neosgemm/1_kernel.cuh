 ...
 */
// __global__ __launch_bounds__(256, 2) void neo_sgemm_kernel(int m, int n, int k, float alpha,
//                                                           const float *a,
...
#define CEIL_(M, N) (((M) + (N)-1) / (N))
#define FLOOR_(M, N) ((M) / (N))

// template <int BLOCK>
// __global__ __launch_bounds__(256, 2) void neo_sgemm_kernel(int M, int N, int K, float alpha,
__global__ void neo_sgemm_kernel(int M, int N, int K, float alpha,
                                                          const float *a,
                                                          const float *b, float beta,
                                                          float *c) {
  ...
  if (_m < M and _n < N) {
    ...
    c[_m * N + _n] = alpha * sum + beta * C[_m * N + _n];
  }
...
void neo_sgemm(int M, int N, int K, float alpha, float *A,
                               float *B, float beta, float *C) {

  // dim3 blockDim(16, 16);
  // dim3 gridDim(CEIL_(M,128),CEIL_(N,128));
  dim3 gridDim(CEIL_DIV(M, 32), CEIL_DIV(N, 32));
  dim3 blockDim(32, 32);

  neo_sgemm_kernel<<<gridDim, blockDim>>>(M, N, K, alpha, A, B, beta, C);
}
