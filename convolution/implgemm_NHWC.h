...
*/
__global__ void implgemm_NHWC(param_t param)
{
...
#pragma unroll
    for (int i = 0; i < 4; ++i)
    {
        ...
        {
            weight_ldg_reg[i] = param.weight_h[weiOffset + tx % 8 + i * weightKOffset];
        }
        else
        {
...
#pragma unroll
    for (int i = 0; i < 4; ++i)
    {
        ...
        {
            input_ldg_reg[i] = param.input_h[inOffset + inOffsetTmp];
        }
        else
        {
            input_ldg_reg[i] = 0.0;
...
#pragma unroll
        for (int i = 0; i < 4; ++i)
        {
            ...
            {
                weight_ldg_reg[i] = param.weight_h[weiOffset + weiOffsetTmp + i * weightKOffset];
            }
            else
            {
                weight_ldg_reg[i] = 0.0;
...
#pragma unroll
    ...
    if (tx < 128)
    {
        smembias[tx] = param.bias_h[by * 128 + tx];
    }
...
void launch_implgemm_NHWC(param_t param)
{
    ...
    implgemm_NHWC<<<grid, block>>>(param);
}
