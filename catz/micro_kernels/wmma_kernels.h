...
{
  // 线程索引和块索引
  int warp_id = (blockIdx.x * blockDim.x + threadIdx.x) / 32; // 计算 warp 的 ID
  int lane_id = threadIdx.x % 32; // 当前线程在 warp 中的索引
  ...
  asm volatile("ldmatrix.sync.aligned.m8n8.x4.row.b16 {%0, %1, %2, %3}, [%4];\n"
