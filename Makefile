...
build:
	@mkdir -p $(BUILD_DIR)
	@cd $(BUILD_DIR) && $(CMAKE) -DCMAKE_BUILD_TYPE=Release .. -DCMAKE_CUDA_COMPILER=nvcc -GNinja
	@ninja -C $(BUILD_DIR)
...
profile: build
	...
	@ncu --set full --export $(BENCHMARK_DIR)/$(PREFIX)kernel_$(KERNEL) --force-overwrite $(BUILD_DIR)/sgemm/sgemm $(KERNEL)

bench: build
	@./$(BUILD_DIR)/sgemm/sgemm ${KERNEL}
