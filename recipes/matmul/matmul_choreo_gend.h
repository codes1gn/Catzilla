...
__global__ void  __choreo_gpu_matmul_parallel(float* lhs, float* rhs, float* output) {
  ...
  for (auto k_tile = I(0); k_tile < make_index<4>(); ++k_tile) {
    ...
    // if (blockIdx.x == 0 & blockIdx.y == 0 & threadIdx.x == 0 && k_tile < I(4)) {
    //   printf("===================== rhs global buff ===================\n");
    //   for (int i = 0; i < 32; i++) {
    //     for (int j = 0; j < 64; j++) {
    //       printf("%.0f ", rhs_mat.tile(Coord(k_tile, q), make_coord(32, 64)).data[i * 128 + j]);
    //     }
    //     printf("\n");
    //   }
    //
    //   printf("===================== rhs local buff ===================\n");
    //   for (int i = 0; i < 32; i++) {
    //     for (int j = 0; j < 64; j++) {
    //       printf("%.0f ", rhs_load__buf__.data[i * 64 + j]);
    //     }
    //     printf("\n");
    //   }
    // }
  ...
  // if (blockIdx.x == 0 & blockIdx.y == 0 & threadIdx.x == 0) {
    ...
    // for (int i = 0; i < 64; i++) {
    //   for (int j = 0; j < 64; j++) {
    //     printf("%.0f ", output_mat.tile(Coord(p, q), make_coord(64, 64)).data[i * 128 + j]);
    //   }
    //   printf("\n");
    // }
    //
    ...
    // for (int i = 0; i < 64; i++) {
    //   for (int j = 0; j < 64; j++) {
    //      assert(output_mat.tile(Coord(p, q), make_coord(64, 64)).data[i * 128 + j] == l2_out.data[i * 64 + j]);
    //   }
    // }
    // printf("equal\n");
  // }
