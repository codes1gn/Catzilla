...
template <const int M_TILE, const int N_TILE, const int K_TILE, const int M_REG,
          ...
          const int Y_THREAD>
__global__ void
_matmul_tensor_cores_wmma_f16f32(int M, int N, int K, float alpha, float *lhs,
...
void matmul_tensor_cores_wmma_f16f32(int M, int N, int K, float alpha, float *A,
                                     float *B, float beta, float *C)
{
  ...
  cudaFuncSetAttribute(
    _matmul_tensor_cores_wmma_f16f32<M_TILE, N_TILE, K_TILE, M_REG, N_REG,
                                     K_REG, X_THREAD, Y_THREAD>,
    cudaFuncAttributePreferredSharedMemoryCarveout,
    cudaSharedmemCarveoutMaxShared);
  _matmul_tensor_cores_wmma_f16f32<M_TILE, N_TILE, K_TILE, M_REG, N_REG, K_REG,
                                   X_THREAD, Y_THREAD>
...
template <const int M_TILE, const int N_TILE, const int K_TILE, const int M_REG,
          ...
          const int Y_THREAD>
__global__ void
_matmul_tensor_cores_mma_f16f32(int M, int N, int K, float alpha, float *lhs,
                                float *rhs, float beta, float *out)
{
  auto lhs_shape = make_coord(M, K);
  auto rhs_shape = make_coord(K, N);
...
void matmul_tensor_cores_mma_f16f32(int M, int N, int K, float alpha, float *A,
                                    float *B, float beta, float *C)
{
  const int M_TILE = 16;
  const int K_TILE = 16;
  ...
  dim3 blockDim(X_THREAD, Y_THREAD);
  cudaFuncSetAttribute(
    _matmul_tensor_cores_mma_f16f32<M_TILE, N_TILE, K_TILE, M_REG, N_REG, K_REG,
                                    X_THREAD, Y_THREAD>,
    cudaFuncAttributePreferredSharedMemoryCarveout,
    cudaSharedmemCarveoutMaxShared);
  _matmul_tensor_cores_mma_f16f32<M_TILE, N_TILE, K_TILE, M_REG, N_REG, K_REG,
...
template <const int M_TILE, const int N_TILE, const int K_TILE, const int M_REG,
          ...
          const int Y_THREAD>
__global__ void
_matmul_tensor_cores_mma_tf32f32(int M, int N, int K, float alpha, float *lhs,
                                 float *rhs, float beta, float *out)
{
  auto lhs_shape = make_coord(M, K);
  auto rhs_shape = make_coord(K, N);
...
#pragma unroll
    ...
    for (int n = 0; n < CEIL_DIV(N_TILE, 8); n++) {
      mma_m16n8k8_tf32f32(out_shared_mat.data, lhs_shared_mat.data,
                          rhs_shared_mat.tile_ex(Coord(0, n), Coord(8, 8)).data,
                          out_shared_mat.data);
    }
    // contract at 128x128x32 micro-kernel
    // mma_m16n8k8_tf32f32(out_shared_mat.data, lhs_shared_mat.data,
...
void matmul_tensor_cores_mma_tf32f32(int M, int N, int K, float alpha, float *A,
                                     float *B, float beta, float *C)
{
  const int M_TILE = 16;
  const int K_TILE = 8;
  ...
  dim3 blockDim(X_THREAD, Y_THREAD);
  cudaFuncSetAttribute(
    _matmul_tensor_cores_mma_tf32f32<M_TILE, N_TILE, K_TILE, M_REG, N_REG,
                                     K_REG, X_THREAD, Y_THREAD>,
    cudaFuncAttributePreferredSharedMemoryCarveout,
    cudaSharedmemCarveoutMaxShared);
  _matmul_tensor_cores_mma_tf32f32<M_TILE, N_TILE, K_TILE, M_REG, N_REG, K_REG,
                                   X_THREAD, Y_THREAD>
    <<<gridDim, blockDim>>>(M, N, K, alpha, A, B, beta, C);
  return;
}
