                                              ...
                                              float beta, float *out)
{
  ...
  auto out_shape_v
    = make_coord(M, CEIL_DIV(N, 4)); // in float4 type, overall shape is M, N/4

  auto lhs_sm_tile_shape = Coord(M_TILE, K_TILE);
  auto rhs_sm_tile_shape_v = Coord(K_TILE, CEIL_DIV(N_TILE, 4));
  auto out_sm_tile_shape_v = Coord(
    M_TILE,
    CEIL_DIV(N_TILE,
             4)); // in float4 type, sm shape is M_tile, N_tile/4 as well
  ...
  MatrixV partial_sum = make_local_v<CEIL_DIV(M_TILE, Y_THREAD),
                                     CEIL_DIV(CEIL_DIV(N_TILE, 4), X_THREAD)>();
...
#pragma unroll
      for (int n = 0; n < CEIL_DIV(CEIL_DIV(N_TILE, 4), X_THREAD); n++) {
        rhs_shared_mat.tile_ex(Coord(kin, n), per_block_data_shape)
          .dist_ex(Coord(threadIdx.y, threadIdx.x))
          = rhs_mat.tile_ex(Coord(ko, blockIdx.x), rhs_sm_tile_shape_v)
    ...
    matmul_kernel_coalesced<M_TILE, N_TILE, K_TILE, Y_THREAD, X_THREAD>(
      (float *)lhs_shared_mat.data, (float *)rhs_shared_mat.data,
...
#pragma unroll
    for (int n = 0; n < CEIL_DIV(CEIL_DIV(N_TILE, 4), X_THREAD); n++) {
      out_mat.tile_ex(Coord(blockIdx.y, blockIdx.x), out_sm_tile_shape_v)
