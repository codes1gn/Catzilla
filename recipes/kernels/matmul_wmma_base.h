...
template <const int M_TILE, const int N_TILE, const int K_TILE, const int M_REG,
          const int N_REG, const int K_REG, const int X_THREAD,
          const int Y_THREAD>
__global__ void _catzilla_matmul_mma_v1(int M, int N, int K, float alpha,
...
#pragma unroll
    ...
    // contract at 128x128x32 micro-kernel
    mma_m16n16k8(lhs_shared_mat.data, rhs_shared_mat.data, out_shared_mat.data);
  }
  __syncthreads();
...
void catzilla_matmul_mma_v1(int M, int N, int K, float alpha, float *A,
                            float *B, float beta, float *C)
{
  const int M_TILE = 16;
  const int K_TILE = 8;
  ...
  dim3 blockDim(X_THREAD, Y_THREAD);
  cudaFuncSetAttribute(
    _catzilla_matmul_mma_v1<M_TILE, N_TILE, K_TILE, M_REG, N_REG, K_REG,
                            X_THREAD, Y_THREAD>,
    cudaFuncAttributePreferredSharedMemoryCarveout,
    cudaSharedmemCarveoutMaxShared);
  _catzilla_matmul_mma_v1<M_TILE, N_TILE, K_TILE, M_REG, N_REG, K_REG, X_THREAD,
                          Y_THREAD>
    <<<gridDim, blockDim>>>(M, N, K, alpha, A, B, beta, C);
  return;
}
