                                    ...
                                    float *out)
{
  ...
  for (int k = 0; k < CEIL_DIV(K, K_TILE); k++) { // range(0, 128, 1)
    ...
    lhs_shared[distribute_(tid_x, tid_y, 1, K_TILE)]
      = __float2half(lhs[tiling_(bid_y, k, M_TILE, K_TILE, K, 1)
                         + distribute_(tid_x, tid_y, 1, K)]);
    ...
    rhs_shared[distribute_(tid_x, tid_y, 1, N_TILE)]
      = __float2half(rhs[tiling_(k, bid_x, K_TILE, N_TILE, N, 1)
                         + distribute_(tid_x, tid_y, 1, N)]);
    __syncthreads();
                        ...
                        float beta, float *C)
{
  ...
  dim3 blockDim(16, 16);
  cudaFuncSetAttribute(_catzilla_matmul_v1<16, 16, 16>,
                       cudaFuncAttributePreferredSharedMemoryCarveout,
                       cudaSharedmemCarveoutMaxShared);
  _catzilla_matmul_v1<16, 16, 16>
